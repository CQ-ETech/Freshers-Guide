...
export default function Timeline() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#150000] to-[#220700] text-white py-12 px-6">
      <h1 className="text-4xl font-bold text-center mb-12 text-[#FFBB00]">Your College Timeline</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {timelineData.map((item, index) => (
          <div
            key={index}
            className="bg-[#2a2a2a] border border-[#4B1000] rounded-2xl p-6 shadow-lg hover:scale-[1.02] transition-all duration-300"
          >
            <h2 className="text-xl font-semibold mb-2 text-[#FFEA00]">{item.title}</h2>
            <p className="text-sm text-gray-400 mb-2">{item.months}</p>
            <p className="text-md text-[#FFF8E4]">{item.note}</p>
          </div>
        ))}
      </div>
